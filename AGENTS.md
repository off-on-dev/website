...
- **Never move focus on page load.** Focus placement after a navigation belongs to the browser here: a full load already resets focus to the document start and announces the new title. A focus-to-`#main-content` block lived in `Layout.astro` as a leftover from `<ClientRouter />`, where a DOM swap left focus stranded and the router had to restore it. In an MPA it is a regression: it drops the skip link and the whole `<header>` nav out of the forward tab order, and moves a screen reader's virtual cursor past both landmarks. It also fires arbitrarily, since `document.referrer` survives a reload but is empty on back/forward. `e2e/smoke.spec.ts` guards its absence and `e2e/a11y.spec.ts` checks the skip link after a real link click, not just a `page.goto`.
