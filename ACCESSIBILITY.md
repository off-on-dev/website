...
- `tabIndex={0}` makes the abbreviation a focusable tooltip trigger so keyboard and touch users can reveal the visual tooltip (WCAG 1.4.13). Escape dismisses it without moving focus (a `dismissed` state drops the focus-shown visibility while keeping focus on the element). The eslint rule `no-noninteractive-tabindex` is suppressed intentionally.
- **Never nest `<Abbr>` inside an `<a>` or `<button>`.** The `tabIndex={0}` creates an orphan tab stop inside an interactive element, which is invalid HTML (interactive content inside interactive content is forbidden). Use a plain `<abbr title="...">` inside a link or button instead.
- For abbreviations inside `dangerouslySetInnerHTML` prose (`.md-inline` / `.md-content` containers): use `<abbr title="...">` in the YAML/markdown source. The generator (`scripts/generate-adventures.mjs`) converts `title` to `data-title` (drives the visual tooltip), adds `tabindex="0"`, and injects an adjacent `<span class="sr-only">` referenced by `aria-describedby` so the expansion reaches screen readers while the visible token stays the accessible name (WCAG 2.5.3). It does **not** use `aria-label`, which would replace the token. This mirrors the `<Abbr>` component.
