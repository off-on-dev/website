...
- `docs-ext-link` class on all inline prose links site-wide. Handles both modes: dark mode foreground text with amber underline, hover to full `#ffc034`; light mode near-black text with `currentColor` underline, hover to `muted-foreground`. Used in `CommunityGuide`, `ChallengeContextSection`, `DiscussionSection`, `CommunitySection`, `LevelCard`, `PersonNameLink`, and `ChallengeBuildersSection`. Do not use `hover:text-primary` or `hover:underline` on inline links — use `docs-ext-link` instead.
