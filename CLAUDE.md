...
Tracked cleanups not yet done: Shiki syntax highlighting for code blocks (`markdown.shikiConfig` is set but the custom markdown pipeline in `src/lib/markdown-pipeline.mjs` doesn't highlight yet); and the component-by-component rewrite of `styleguide.md` (a current component inventory has been added in the interim). (Done: lint, sitemap endpoint, consent runtime tests `e2e/consent.spec.ts`, gated click-event tracking, the full React-parity restoration of the home/adventures/challenges pages + nav/footer chrome, the code-block header + Copy button, and the abbr JS tooltip — a `position:fixed` portal in `Layout.astro` that clamps to the viewport and escapes overflow clipping.)
