...
// temp tree. The module reads process.cwd() at import time, so the spy
// must be active before the dynamic import. Using vi.spyOn ensures the spy
// is properly registered with Vitest and mockRestore() always runs, even if
// the import itself throws.
let getDiscussion: typeof import("@/lib/community-data").getDiscussion;
let getLeaderboard: typeof import("@/lib/community-data").getLeaderboard;

beforeAll(async () => {
  const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(tempRoot);
  ({ getDiscussion, getLeaderboard } = await import("@/lib/community-data"));
  cwdSpy.mockRestore();

  mkdirSync(join(tempRoot, adventureId), { recursive: true });
...
// Resolve the paths used by community-data.
