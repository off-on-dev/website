...
export type AdventureLevel = {
  ...
  /** Short technical premise: what you need to build. One string per paragraph. */
  intro?: string[];
  /** Narrative paragraphs that set the scene. One string per paragraph. */
  backstory?: string[];
  /** System architecture and environment overview. One string per paragraph. */
  architecture?: string[];
  /** Optional diagram image for the architecture section. Import the file and assign the URL, e.g. `import diagram from "@/assets/diagrams/my-diagram.svg"`. */
  architectureDiagram?: string;
  /** What the learner needs to accomplish to complete the level. */
  objective?: string[];
  /** Tools, services, and components the learner will work with. name is the tool title; description explains it. */
  toolbox?: { name: string; description?: string; url?: string }[];
  /** Step-by-step guide explaining how to approach and complete the level. */
  howToPlay?: string[];
}
    ...
    levels: [
      {
        ...
        howToPlay: [
          ...
          "Open the Ports tab, find port 30100 (Argo CD), and log in with username 'readonly' and password 'a-super-secure-password'.",
          "Fix the ApplicationSet at adventures/01-echoes-lost-in-orbit/beginner/manifests/appset.yaml, then apply it: kubectl apply -n argocd -f adventures/01-echoes-lost-in-orbit/beginner/manifests/appset.yaml",
          "Run the smoke test at adventures/01-echoes-lost-in-orbit/beginner/smoke-test.sh to verify your solution locally.",
          "Commit and push your changes to your fork, then trigger the verification workflow on GitHub Actions to officially claim completion.",
        ],
      },
      {
        id: "intermediate",
        ...
        howToPlay: [
          ...
          "Open the Ports tab. Log into Argo CD at port 30100 (username: readonly, password: a-super-secure-password). Access Argo Rollouts at port 30101 and Prometheus at port 30102.",
          "Review and fix the configuration in adventures/01-echoes-lost-in-orbit/intermediate/manifests/",
          "Commit and push your changes: git add adventures/01-echoes-lost-in-orbit/intermediate/manifests/ then commit and push. Optionally speed up Argo CD sync: argocd app get echo-server-staging --refresh and argocd app get echo-server-prod --refresh.",
          "After Argo CD syncs, retry the rollouts: kubectl argo rollouts retry rollout echo-server -n echo-staging and kubectl argo rollouts retry rollout echo-server -n echo-prod. The rollout should progress through canary stages (33% to 66% to 100%).",
          "Run the smoke test: adventures/01-echoes-lost-in-orbit/intermediate/smoke-test.sh",
          "Commit and push your changes to your fork, then trigger the verification workflow on GitHub Actions to officially claim completion.",
        ],
      },
      {
        ...
        howToPlay: [
          ...
          "Open the Ports tab. Log into Argo CD at port 30100 (username: readonly, password: a-super-secure-password). Access Argo Rollouts at 30101, Prometheus at 30102, and Jaeger at 30103.",
          "Fix the manifests in adventures/01-echoes-lost-in-orbit/expert/manifests/",
          "Commit and push your changes. Refresh Argo CD apps: argocd app get hotrod --refresh and argocd app get otel --refresh. If you changed HotROD, retry the rollout: kubectl argo rollouts retry rollout hotrod -n hotrod. If you changed the OTel Collector config, restart it: kubectl rollout restart daemonset/collector -n otel.",
          "Watch rollout progress: kubectl argo rollouts get rollout hotrod -n hotrod --watch. The rollout should progress automatically based on analysis metrics.",
          "Run the smoke test: adventures/01-echoes-lost-in-orbit/expert/smoke-test.sh",
          "Commit and push your changes to your fork, then trigger the verification workflow on GitHub Actions to officially claim completion.",
        ],
      }
    ]
    ...
    levels: [
      {
        ...
        howToPlay: [
          ...
          "Open the Ports tab, find the GCP API Mock at port 30104. Use it to explore the mock cloud resources created by your configuration.",
          "All OpenTofu files are in adventures/02-building-cloudhaven/beginner/. Run grep -r \"TODO\" . to find everything left by the previous engineer. Files to review: main.tf, state.tf, variables.tf, merchants.tf, audit.tf, outputs.tf.",
          "After fixing the TODOs, run tofu apply. If you changed the backend configuration, run tofu init -migrate-state first.",
      ...
      },
      {
        ...
        howToPlay: [
          "Prefer working locally? Clone the repo and open it in any editor that supports the Dev Containers specification (VS Code, JetBrains IDEs, and others). The devcontainer config will be detected automatically.",
          "Wait ~2 minutes for the environment to initialize.",
          "Open the Ports tab, find the GCP API Mock at port 30104. Use it to explore mock cloud resources.",
          "All files are in adventures/02-building-cloudhaven/intermediate/. Run make test to see which tests fail. The tests define the expected behaviour: fix the implementation in modules/district/ to match them. Complete the integration test in tests/integration.tftest.hcl.",
          "Run make test (all tests should pass) then make apply.",
          "Run the smoke test: ./smoke-test.sh",
          "Commit and push your changes to your fork, then trigger the verification workflow on GitHub Actions to officially claim completion.",
        ],
      },
      {
        ...
        howToPlay: [
          "Prefer working locally? Clone the repo and open it in any editor that supports the Dev Containers specification (VS Code, JetBrains IDEs, and others). The devcontainer config will be detected automatically.",
          "Wait ~2 minutes for the environment to initialize.",
          "Open the Ports tab, find the GCP API Mock at port 30104. The port is set to public so GitHub Actions runners can reach the mock API during workflow runs.",
          "Fix the three workflows in .github/workflows/: adventure02-expert-detect-drift.yaml, adventure02-expert-validate-changes.yaml, and adventure02-expert-apply-infrastructure.yaml. The OpenTofu configuration is correct, focus only on the workflow files.",
          "Commit and push to main. To trigger drift detection, go to the Actions tab, select the drift detection workflow, and click Run workflow. The infrastructure has intentional drift, so the workflow should create a draft PR.",
          "Click Ready for Review on the draft PR to trigger the validation workflow. To re-trigger validation after pushing new changes, convert the PR back to draft then ready for review again.",
          "When the PR is merged to main, the apply workflow runs automatically.",
          "Run the smoke test: cd adventures/02-building-cloudhaven/expert && ./smoke-test.sh",
          "Commit and push your changes to your fork, then trigger the verification workflow on GitHub Actions to officially claim completion.",
        ],
      }
    ]
    ...
    levels: [
      {
        ...
        howToPlay: [
          ...
          "Open the Ports tab, find Jaeger at port 30103. This is where you will analyze the traces sent by HubSystem.",
          "Add OpenTelemetry instrumentation to hubsystem.py using OpenLLMetry. The OTel Collector and Jaeger are already configured correctly.",
          "Run make hubsystem to start the application. Interact with the AI to generate traces, then check Jaeger.",
          "Find the trace responsible for the high bandwidth usage and inspect its attributes to answer quiz.txt.",
          "Run ./verify.sh. If it passes, follow the on-screen instructions to commit your changes and generate a Certificate of Completion. Paste the certificate into the challenge thread to claim your victory.",
        ],
      },
      {
        id: "intermediate",
        ...
        howToPlay: [
          ...
          "Open the Ports tab: Prometheus at port 30102, Jaeger at port 30103.",
          "Instrument art.py with OpenLLMetry and add the custom metric. Update manifests/prometheus-rule.yaml with the Distraction Ratio recording rule. After changing the rule file, run make apply.",
          "Run make art to interact with ART, or make traffic to generate continuous traffic for your metric graphs.",
          "Verify traces in Jaeger and the recording rule in Prometheus. Fix the navigation system so ART returns jump coordinates.",
          "Run ./verify.sh. If it passes, follow the on-screen instructions to commit your changes and generate a Certificate of Completion. Paste the certificate into the challenge thread to claim your victory.",
        ],
      },
      {
        ...
        howToPlay: [
          "Prefer working locally? Clone the repo and open it in any editor that supports the Dev Containers specification (VS Code, JetBrains IDEs, and others). The devcontainer config will be detected automatically.",
          "Wait ~15 minutes for all infrastructure to initialize.",
          "Open the Ports tab, find Jaeger at port 30103. Verify your spans look correct and that sampling works as expected.",
          "Fix the chat span in art.py to follow OpenTelemetry GenAI semantic conventions including token usage. Fix the tail sampling config in manifests/otel-collector-config.yaml.",
          "After changing art.py, restart make traffic to pick up new instrumentation. After changing the collector config, apply it: kubectl apply -f manifests/otel-collector-config.yaml -n otel && kubectl rollout restart deployment/collector -n otel.",
          "Run make traffic to generate traces. Verify in Jaeger that spans follow conventions and only errors and slow traces appear.",
          "Run ./verify.sh. If it passes, follow the on-screen instructions to commit your changes and generate a Certificate of Completion. Paste the certificate into the challenge thread to claim your victory.",
        ],
      }
    ]
    ...
    levels: [
      {
        ...
        howToPlay: [
          "Prefer working locally? Clone the repo and open it in any editor that supports the Dev Containers specification (VS Code, JetBrains IDEs, and others). The devcontainer config will be detected automatically.",
          "Start the lab: run ./mvnw spring-boot:run from the terminal, or press F5 in VS Code with Laboratory.java open. The lab starts in the broken state, returning the hard-coded 'untreated' response.",
          "Open the Ports tab, set port 8080 to Public, then click the forwarded address to confirm the hard-coded 'untreated' response.",
          ...
          "Restart the lab. Run curl -s http://localhost:8080/ | jq and confirm the value resolves from flags.json. Then edit flags.json, change defaultVariant, save, and re-run curl without restarting anything to verify hot-reload.",
          "Run ./verify.sh. If it passes, follow the on-screen instructions to commit your changes and generate a Certificate of Completion. Paste the certificate into the challenge thread to claim your victory.",
        ],
      },
      {
        ...
        howToPlay: [
          "Prefer working locally? Clone the repo and open it in any editor that supports the Dev Containers specification (VS Code, JetBrains IDEs, and others). The devcontainer config will be detected automatically.",
          "Wait ~2-3 minutes for the Java toolchain to install.",
          "Start the lab: run ./mvnw spring-boot:run or click Run on Laboratory in the Spring Boot Dashboard. Confirm the broken state: curl 'http://localhost:8080/?species=zyklop' returns 'blurry' (no targeting fires yet). Stop the app and start fixing.",
          "Create a Spring HandlerInterceptor named SpeciesInterceptor: in preHandle, read ?species= and put it on the transaction context; in afterCompletion, clear the context so values do not leak across pooled threads. Register a ThreadLocalTransactionContextPropagator once on the OpenFeature API in a static initialiser.",
          "Update OpenFeatureConfig to: register SpeciesInterceptor with Spring, read the COUNTRY environment variable and set it as the global evaluation context, and register AuditHook globally on the OpenFeature API.",
          "Update Trial so each evaluation passes dose on the invocation context ('standard' by default, overridable via ?dose=).",
          "Implement AuditHook: in after(), write an [AUDIT] log line with flag name, variant, reason, and a fixed allowlist of attributes (species, country, dose). Log at WARN when the variant is 'clouded', otherwise INFO. Implement error() so failed evaluations are not silent.",
          "Run the lab with ./run-germany.sh (COUNTRY=de) or ./run-austria.sh (COUNTRY=at). Test each targeting branch: /?species=zyklop returns 'enhanced', /?dose=standard returns 'sharp' in Germany, /?dose=underdose returns 'clouded', /?species=zyklop&dose=underdose returns 'enhanced' (species wins).",
          "Run ./verify.sh. If it passes, follow the on-screen instructions to commit your changes and generate a Certificate of Completion. Paste the certificate into the challenge thread to claim your victory.",
        ],
      },
    ],
