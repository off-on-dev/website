  ...
  levels: [
    {
      ...
      objective: [
        "curl <a href=\"http://localhost:8080/\" target=\"_blank\" rel=\"noopener noreferrer\" aria-describedby=\"new-tab-hint\">http://localhost:8080/</a> returns a vision_state reading resolved from flags.json (not the hard-coded 'untreated' fallback)",
...
<pre tabindex="0" aria-label="Code block"><code class="language-sh">curl -s http://localhost:8080/ | jq
    ...
    },
    {
      ...
      toolbox: [
        ...
        { name: "curl", description: "sends requests to <a href=\"http://localhost:8080/\" target=\"_blank\" rel=\"noopener noreferrer\" aria-describedby=\"new-tab-hint\">http://localhost:8080/</a> to test each targeting branch", url: "https://curl.se/" },
      ...
      howToPlay: [
        ...
        { title: "Explore the UIs", content: `<p>Open the <strong>Ports</strong> tab and navigate to each service:</p>
<ul>
<li><strong>Port 8080:</strong> Spring Boot lab. The application under test. Access via the Ports tab or curl <a href="http://localhost:8080/" target="_blank" rel="noopener noreferrer" aria-describedby="new-tab-hint">http://localhost:8080/</a>.</li>
</ul>` },
...
<p>You should see one <code>[AUDIT] flag=vision_state variant=... reason=... species=... country=... dose=...</code> line per request. <code>clouded</code> outcomes log at WARN.</p>` },
    ...
    },
    {
      ...
      toolbox: [
        ...
        { name: "./mvnw", description: "Spring Boot Maven Wrapper, no global Maven install required" },
        { name: "curl", description: "sends requests to <a href=\"http://localhost:8080/\" target=\"_blank\" rel=\"noopener noreferrer\" aria-describedby=\"new-tab-hint\">http://localhost:8080/</a> to test the lab, and to Prometheus on <a href=\"http://localhost:9090/\" target=\"_blank\" rel=\"noopener noreferrer\" aria-describedby=\"new-tab-hint\">http://localhost:9090/</a> to query metrics directly", url: "https://curl.se/" },
        { name: "Grafana", description: "browser UI at <a href=\"http://localhost:3000\" target=\"_blank\" rel=\"noopener noreferrer\" aria-describedby=\"new-tab-hint\">http://localhost:3000</a> (admin/admin) for the Feature Flag Metrics dashboard and Tempo trace explorer" },
        { name: "jq", description: "pretty-prints the JSON evaluation details", url: "https://jqlang.org/" },
      ],
      howToPlay: [
...
<li><strong>Port 9090:</strong> Prometheus. Query metrics directly via the Prometheus UI or curl <a href="http://localhost:9090/api/v1/query" target="_blank" rel="noopener noreferrer" aria-describedby="new-tab-hint">http://localhost:9090/api/v1/query</a>.</li>
